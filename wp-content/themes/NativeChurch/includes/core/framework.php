...
/**
        Still need to port these.
            $defaults['footer_credit']      = '<span id="footer-thankyou">' . __( 'Options panel created using', 'framework') . ' <a href="' . $this->framework_url . '" target="_blank">' . __('IMIC Framework', 'framework') . '</a> v' . self::$_version . '</span>';
            $defaults['help_tabs']          = array();
            $defaults['help_sidebar']       = ''; // __( '', 'framework' );
...
**/
            self::$_properties = array( 
                'args' => array(
                    ...
                    'menu_title' => array( 
                            ...
                            'default'=> __( 'IMIC Options', 'framework' )
                        ),              
                    'page_title' => array( 
                            'data_type'=>'string', 
                            'label'=>'Page Title', 
        ...
        public function __construct( $sections = array(), $args = array(), $extra_tabs = array() ) {
            ...
            $defaults['menu_title']         = __( 'IMIC Options', 'framework' );
            $defaults['page_icon']          = 'icon-themes';
            $defaults['page_title']         = __( 'IMIC Options', 'framework' );
...
//                                $class_file = apply_filters( 'imic/field/class/'.$field['type'], self::$_dir . 'inc/fields/' . $field['type'] . '/field_' . $field['type'] . '.php', $field ); // REMOVE
        ...
        public function _options_page_html() {
            ...
            echo '<a href="javascript:void(0);" id="expand_options">' . __( 'Expand', 'framework' ) . '</a>';
            echo '<div class="imic-action_bar">';
            submit_button( __( 'Save Changes', 'framework'), 'primary', 'imic_save', false );
            echo '&nbsp;';
            submit_button( __( 'Reset to Defaults', 'framework' ), 'secondary', $this->args['opt_name'] . '[defaults]', false );
            echo '</div>';
            echo '<div class="imic-ajax-loading" alt="' . __( 'Working...', 'framework' ) . '">&nbsp;</div>';
            echo '<div class="clear"></div>';
            echo '</div>';
            // Warning bar
            if( isset( $_GET['settings-updated'] ) && $_GET['settings-updated'] == 'true' && $saved == '1' ) {
                if( isset( $this->options['IMIC_imported'] ) && $this->options['IMIC_imported'] === 1 ) {
                    echo '<div id="imic-imported">' . apply_filters( 'imic-imported-text-' . $this->args['opt_name'], '<strong>' . __( 'Settings Imported!', 'framework' ) ) . '</strong></div>';
                } else {
                    echo '<div id="imic-save">' . apply_filters( 'imic-saved-text-' . $this->args['opt_name'], '<strong>'.__( 'Settings Saved!', 'framework' ) ) . '</strong></div>';
                }
            }
            echo '<div id="imic-save-warn">' . apply_filters( 'imic-changed-text-' . $this->args['opt_name'], '<strong>'.__( 'Settings have changed, you should save them!', 'framework' ) ) . '</strong></div>';
            echo '<div id="imic-field-errors"><strong><span></span> ' . __( 'error(s) were found!', 'framework' ) . '</strong></div>';
            echo '<div id="imic-field-warnings"><strong><span></span> ' . __( 'warning(s) were found!', 'framework' ) . '</strong></div>';
            echo '</div>';
            echo '<div class="clear"></div>';
            // Sidebar
            ...
            if( $this->args['show_import_export'] === true ) {
                ...
                echo '<a href="javascript:void(0);" id="import_export_default_section_group_li_a" class="imic-group-tab-link-a" data-rel="import_export_default">' . $icon . ' <span class="group_title">' . __( 'Import / Export', 'framework' ) . '</span></a>';
                echo '</li>';
            ...
            if( $this->args['dev_mode'] === true ) {
                ...
                echo '<a href="javascript:void(0);" id="dev_mode_default_section_group_li_a" class="imic-group-tab-link-a custom-tab" data-rel="dev_mode_default">' . $icon . ' <span class="group_title">' . __( 'Options Object', 'framework' ) . '</span></a>';
                echo '</li>';
            }
            if( $this->args['system_info'] === true ) {
                ...
                echo '<a href="javascript:void(0);" id="system_info_default_section_group_li_a" class="imic-group-tab-link-a custom-tab" data-rel="system_info_default">' . $icon . ' <span class="group_title">' . __( 'System Info', 'framework' ) . '</span></a>';
                echo '</li>';
            }
            echo '</ul>';
            echo '</div>';
            ...
            if( $this->args['show_import_export'] === true ) {
                ...
                echo '<h3>' . __( 'Import / Export Options', 'framework' ) . '</h3>';
				echo '<h4>' . __( 'Import Options', 'framework' ) . '</h4>';
                echo '<p><a href="javascript:void(0);" id="imic-import-code-button" class="button-secondary">' . __( 'Import from file', 'framework' ) . '</a> <a href="javascript:void(0);" id="imic-import-link-button" class="button-secondary">' . __( 'Import from URL', 'framework' ) . '</a></p>';
                echo '<div id="imic-import-code-wrapper">';
                echo '<div class="imic-section-desc">';
                echo '<p class="description" id="import-code-description">' . apply_filters( 'imic-import-file-description', __( 'Input your backup file below and hit Import to restore your sites options from a backup.', 'framework' ) ) . '</p>';
                echo '</div>';
                ...
                echo '<div class="imic-section-desc">';
                echo '<p class="description" id="import-link-description">' . apply_filters( 'imic-import-link-description', __( 'Input the URL to another sites options set and hit Import to load the options from that site.', 'framework' ) ) . '</p>';
                echo '</div>';
                echo '<input type="text" id="import-link-value" name="' . $this->args['opt_name'] . '[import_link]" class="large-text noUpdate" value="" />';
                echo '</div>';
                echo '<p id="imic-import-action"><input type="submit" id="imic-import" name="' . $this->args['opt_name'] . '[import]" class="button-primary" value="' . __( 'Import', 'framework' ) . '">&nbsp;&nbsp;<span>' . apply_filters( 'imic-import-warning', __( 'WARNING! This will overwrite all existing option values, please proceed with caution!', 'framework' ) ) . '</span></p>';
                echo '<div class="hr"/><div class="inner"><span>&nbsp;</span></div></div>';
                echo '<h4>' . __( 'Export Options', 'framework' ) . '</h4>';
                echo '<div class="imic-section-desc">';
                echo '<p class="description">' . apply_filters( 'imic-backup-description', __( 'Here you can copy/download your current option settings. Keep this safe as you can use it as a backup should anything go wrong, or you can use it to restore your settings on this site (or any other site).', 'framework' ) ) . '</p>';
                echo '</div>';
                /** @noinspection PhpUndefinedConstantInspection */
                echo '<p><a href="javascript:void(0);" id="imic-export-code-copy" class="button-secondary">' . __( 'Copy', 'framework' ) . '</a> <a href="' . add_query_arg( array( 'feed' => 'imicopts-' . $this->args['opt_name'], 'action' => 'download_options', 'secret' => md5( AUTH_KEY . SECURE_AUTH_KEY ) ), site_url() ) . '" id="imic-export-code-dl" class="button-primary">' . __( 'Download', 'framework' ) . '</a> <a href="javascript:void(0);" id="imic-export-link" class="button-secondary">' . __( 'Copy Link', 'framework' ) . '</a></p>';
    ...
    echo "hello";
     echo '</div>';
            ...
            if( $this->args['dev_mode'] === true ) {
                ...
                echo '<h3>' . __( 'Options Object', 'framework' ) . '</h3>';
                echo '<div class="imic-section-desc">';
                echo '<div id="imic-object-browser"></div>';
                echo '</div>';
                echo '<div id="imic-object-json" class="hide">'.json_encode($this->options).'</div>';
                echo '<a href="#" id="consolePrintObject" class="button">' . __( 'Show Object in Javascript Console Object', 'framework' ) . '</a>';
            ...
            if( $this->args['system_info'] === true ) {
                ...
                echo '<h3>' . __( 'System Info', 'framework' ) . '</h3>';
