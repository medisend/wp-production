...
if(!function_exists('imic_get_data_by_path')){
        function imic_get_data_by_path($id,$imic_custom_read_more) {
           ...
           if (!empty($slug_thumbnail_id)) {
                echo '<div class="col-md-4 col-sm-4 featured-block"> <a href="' . get_permalink($slug_data->ID) . '" class="img-thumbnail"> <img src="' . $src[0] . '" alt="' . $slug_data->post_title . '"> <strong>' . $read_More_text . '</strong> <span class="more">' . __('read more', 'framework') . '</span> </a> </div>';
            }
        }  
}
?>