function embedSelectedShortcode() {
	...
	if (current_shortcode != -1) {
		...
		// Pagination Tags
		var pagination_size = document.getElementById('pagination-size').value;
		var pagination_number = document.getElementById('pagination-number').value;		
		...
		if (shortcode_select == 'shortcode-staff') {
			shortcodeHTML = '<br/>[staff number="'+staff_number+'" order="'+staff_order+'"]<br/>';	
		}
		...
		/////////////////////////////////////////
		////	PAGINATION SHORTCODE OUTPUT
		/////////////////////////////////////////
		if (shortcode_select == 'shortcode-pagination') {
			shortcodeHTML = '<br/>[pagination size="'+ pagination_size +'"]<br/>';
			
			shortcodeHTML += '[page_first]<br/>';
			index = 0;
			for ( var hc = 0; hc < pagination_number; hc++ ) {
				if(index==0){ pageClass='active'; }else{ pageClass=''; }
				shortcodeHTML += '[page class="'+ pageClass +'"]' + parseInt(hc + 1) + '[/page]<br/>';
				index++;
			}
			shortcodeHTML += '[page_last]<br/>';
						
			shortcodeHTML += '[/pagination]<br/>';
	}
	/////////////////////////////////////////
